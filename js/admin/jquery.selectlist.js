 ...
 */
 ...
 ;(function($,window,document){
    ...
    SelectList.prototype={
		...
		//获取原生选择列表所有option的class,返回数组
        getOptionClass: function(element){
            var that = this,
                $this = $(element),
                $option = $this.children(),
                optionClass = [],
            ...
            for(var i = 0; i < selectLength; i++ ){
				if(typeof($option.eq(i).attr("attr"))!="undefined"){
					optionClass[i] = $option.eq(i).attr("attr");
				}else{
					optionClass[i]='1';
				}
        ...
        renderSelect: function(element){
            var that = this,
				...
				selectOptionClass = that.getOptionClass($this),
                selectedIndex = that.getSelectedIndex($this),
            ...
            for(var i=0; i<selectLength; i++){
                if(i !== selectedIndex){
                    selectListHTML = selectListHTML + '<li class="'+selectOptionClass[i]+'" data-value="' + selectOptionValue[i] + '">' + selectOptionText[i] + '</li>';
                }else{
                    selectListHTML = selectListHTML + '<li   data-value="' + selectOptionValue[i] + '" class="selected '+selectOptionClass[i]+'">' + selectOptionText[i] + '</li>';
                }
        ...
        setSelectEvent: function(element){
            ...
            if(that.settings.enable){
                ...
                selectItem.on('click',function(event){
					...
					//我的处理
					if($(this).parent().parent().parent('#edu').length){
						var aid=$(this).attr('data-value');
						$('.firsttarea_id').val(aid);
						//子级的隐显
						$('#salary').children(".select-list").find("li").hide();
						$('#salary').children(".select-list").find(".parent_id"+aid).show();
						
						if($('#salary').children(".select-list").find(".parent_id"+aid).eq(0).length){
							var vt=$('#salary').children(".select-list").find(".parent_id"+aid).eq(0).text();
							...
							$('#salary').children(".select-list").find(".parent_id"+aid).eq(0).addClass('selected');
						}else{
							$('#salary').children(".select-button").text('没有地区');
							$('#salary').children(".select-button").val('没有地区');
							$('#salary').children(".select-list").find("li").removeClass('selected');
						...
						//var eclass=$(this).attr('data-value');
						//if(eid=='edu'){
                    ...
                    if($(this).parent().parent().parent('#salary1').length){
						var aid=$(this).attr('data-value');
						$('.room_parent_id').val(aid);
						//子级的隐显
					 
						$('#salary2').children(".select-list").find("li").hide();
						$('#salary2').children(".select-list").find(".parent_id"+aid).show();
						
						if($('#salary2').children(".select-list").find(".parent_id"+aid).eq(0).length){
							var vt=$('#salary2').children(".select-list").find(".parent_id"+aid).eq(0).text();
						
							$('#salary2').children(".select-button").text(vt);
							$('#salary2').children(".select-button").val(vt);
							$('#salary2').children(".select-list").find("li").removeClass('selected');
							$('#salary2').children(".select-list").find(".parent_id"+aid).eq(0).addClass('selected');
						}else{
							$('#salary2').children(".select-button").text('没有房间类型');
							$('#salary2').children(".select-button").val('没有房间类型');
							$('#salary2').children(".select-list").find("li").removeClass('selected');
